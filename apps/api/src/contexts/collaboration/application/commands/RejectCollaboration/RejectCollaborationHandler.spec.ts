import { RejectCollaborationHandler } from './RejectCollaborationHandler';
...
describe('RejectCollaborationHandler', () => {
  it('throws when collaboration does not exist', async () => {
    ...
    const handler = new RejectCollaborationHandler(collaborations, publisher);

    await expect(
      handler.execute(new RejectCollaborationCommand('collab-1', 'pro-2')),
  ...
  it('saves and publishes events', async () => {
    ...
    await handler.execute(new RejectCollaborationCommand('collab-1', 'pro-2'));
